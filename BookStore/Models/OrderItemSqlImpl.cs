...
namespace BookStore.Models
{
    public class OrderItemSqlImpl : IOrderItemRepository
    {
        ...
        public List<OrderBooks> GetOrderBooks()
        {
            ...
            OrderItem orderItem;
            Book book;
            comm.CommandText = "select * from [Order] O,OrderItem OT, Book B where O.OrderId = OT.OrderId and OT.BookId = B.BookId";
            conn.Open();
            comm.Connection = conn;
            SqlDataReader reader = comm.ExecuteReader();
            while (reader.Read())
        ...
        //get by orderId
        public List<OrderBooks> GetOrderBooksById(int GOrderId)
        {
            List<OrderBooks> orderbooks = new List<OrderBooks>();
            Order order;
            OrderItem orderItem;
            Book book;
            comm.CommandText = "select * from [Order] O,OrderItem OT, Book B where O.OrderId = OT.OrderId and OT.BookId = B.BookId and O.OrderId="+GOrderId;
            conn.Open();
            comm.Connection = conn;
            SqlDataReader reader = comm.ExecuteReader();
            while (reader.Read())
